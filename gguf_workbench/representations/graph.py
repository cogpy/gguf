...
class GraphRepresentation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "edges": {e_id: e.to_dict() for e_id, e in self.edges.items()},
    ...
    def from_tiny_transformer(cls, model_path: Optional[Path] = None) -> "GraphRepresentation":
        ...
        g.add_node(
            Node(id="embed_op", type="operation", properties={"operation": "embedding_lookup"})
        )

        g.add_node(Node(id="embeddings", type="tensor", shape=(None, 5, 5), dtype="float32"))
        ...
        for i, proj in enumerate(["query", "key", "value"]):
            ...
            g.add_node(Node(id=op_id, type="operation", properties={"operation": "linear"}))

            g.add_node(Node(id=tensor_id, type="tensor", shape=(None, 5, 5), dtype="float32"))
        ...
        g.add_node(Node(id="attn_scores", type="tensor", shape=(None, 5, 5), dtype="float32"))
        ...
        # Softmax
        g.add_node(
            Node(id="softmax_op", type="operation", properties={"operation": "softmax", "axis": -1})
        )

        g.add_node(Node(id="attn_weights", type="tensor", shape=(None, 5, 5), dtype="float32"))
        ...
        # Apply attention
        g.add_node(Node(id="apply_attn_op", type="operation", properties={"operation": "matmul"}))

        g.add_node(Node(id="attn_output_pre", type="tensor", shape=(None, 5, 5), dtype="float32"))
        ...
        g.add_node(Node(id="attn_proj_op", type="operation", properties={"operation": "linear"}))

        g.add_node(Node(id="attn_output", type="tensor", shape=(None, 5, 5), dtype="float32"))
        ...
        # Residual connection
        g.add_node(Node(id="residual1_op", type="operation", properties={"operation": "add"}))

        g.add_node(Node(id="attn_residual", type="tensor", shape=(None, 5, 5), dtype="float32"))
    ...
    def export_graphviz(self, filepath: Optional[Path] = None) -> str:
        ...
        for n_id, node in self.nodes.items():
            ...
            lines.append(
                f'  "{n_id}" [label="{label}", shape={shape}, fillcolor="{color}", style=filled];'
            )
