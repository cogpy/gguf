...
class SymbolicRepresentation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "expressions": {
                name: expr.to_dict() for name, expr in self.expressions.items()
            },
    ...
    def to_latex(self, filepath: Optional[Path] = None) -> str:
        ...
        for param in self.parameters.values():
            ...
            lines.append(f"  {param.name} &\\in \\mathbb{{R}}^{{{shape_str}}} "
    ...
    def from_tiny_transformer(cls, model_path: Optional[Path] = None) -> 'SymbolicRepresentation':
        ...
        sr.add_expression(SymbolicExpression(
            name="h_0",
            expression="E[x]",
            dependencies=["E", "x"],
            properties={
