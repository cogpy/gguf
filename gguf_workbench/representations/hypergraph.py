...
class HypergraphRepresentation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "vertices": {
                v_id: v.to_dict() for v_id, v in self.vertices.items()
            },
            "hyperedges": {
                e_id: e.to_dict() for e_id, e in self.hyperedges.items()
            },
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        hyperedge_sizes = [
            len(e.sources) + len(e.targets) for e in self.hyperedges.values()
        ]
    ...
    def from_tiny_transformer(cls, model_path: Optional[Path] = None) -> 'HypergraphRepresentation':
        ...
        for proj in ["query", "key", "value"]:
            hg.add_vertex(Vertex(
                ...
                shape=(5, 5),
                dtype="float32",
                properties={"description": f"Attention {proj} projection matrix", "parameter_count": 25}
            ))
        ...
        hg.add_vertex(Vertex(
            ...
            shape=(5, 5),
            dtype="float32",
            properties={"description": "Attention output projection matrix", "parameter_count": 25}
        ))
        ...
        hg.add_vertex(Vertex(
            ...
            shape=(5, 10),
            dtype="float32",
            properties={"description": "Output projection to vocabulary", "parameter_count": 50}
        ))
    ...
    def export_graphviz(self, filepath: Optional[Path] = None) -> str:
        ...
        for e_id, edge in self.hyperedges.items():
            lines.append(f'  "{e_id}" [label="{edge.operation}", shape=diamond, fillcolor="orange", style=filled];')
