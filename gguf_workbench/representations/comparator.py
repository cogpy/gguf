...
class RepresentationComparator:
        ...
        for dim in dimensions:
            if dim == "overall_score":
                sorted_metrics = sorted(
                    self.metrics.items(), key=lambda x: x[1].overall_score(), reverse=True
                )
            else:
                sorted_metrics = sorted(
                    self.metrics.items(), key=lambda x: getattr(x[1], dim), reverse=True
                )
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        best_overall = max(self.metrics.items(), key=lambda x: x[1].overall_score())

        best_completeness = max(self.metrics.items(), key=lambda x: x[1].completeness_score)

        best_transparency = max(self.metrics.items(), key=lambda x: x[1].transparency_score)

        best_efficiency = max(self.metrics.items(), key=lambda x: x[1].efficiency_score)
    ...
    def to_markdown(self, filepath: Optional[Path] = None) -> str:
        ...
        lines.append("")
        lines.append(
            "| Representation | Completeness | Transparency | Efficiency | "
            "Expressiveness | Tool Support | Overall |"
        )
        lines.append(
            "|----------------|--------------|--------------|------------|"
            "----------------|--------------|---------|"
